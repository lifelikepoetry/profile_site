...
export default function Footer() {
    ...
    const handleClick = (item: FooterItem) => {
        if (item.title === "Gmail") {
          ...
          toast.success(tMsg("gmail-copy"));
        } else if (item.title === "WeChat") {
          // 微信复制到剪贴板
          navigator.clipboard.writeText(item.href);
          toast.success(tMsg("wechat-copy"));
        } else {
    ...
    return (
        <div className="flex gap-4 items-center justify-between border-t-1 border-white/20 py-2 px-6 w-full z-10">
            <span className="text-white/80 text-sm">{t('copyright')}</span>
